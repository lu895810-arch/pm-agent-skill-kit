...
1. **场景设定图**（`cyx-shuohao-art` 的 `images/<场景名>-sheet.png`）——必挂，环境、材质、光的唯一标准
2. **画内每个角色的设定图**（`cyx-shuohao-characters` 的 `images/<角色名>-sheet.png`）——有几个挂几个
3. **画内叙事道具的设定图**（`cyx-shuohao-art` 的 `images/<道具名>-sheet.png`）——有就挂

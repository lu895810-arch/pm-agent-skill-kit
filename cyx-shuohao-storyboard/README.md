...
给 AI 短剧出**分镜**：把 cyx-shuohao-script 的节拍流切成可以直接下单给视频模型的生成任务单。这是管线里第一个直接面对视频模型的层，前提刻在骨子里：**镜头是生成出来的，多切一刀的成本几乎为零**，短剧观众的注意力节奏是 3 秒左右一切。所以结构是三层：
...
- **分镜图是资产合成，不是凭空画** — 出图挂场景/角色/道具设定图当参考图，cyx-shuohao-art 和 cyx-shuohao-characters 的图在这一步真正被消费。有 codex 就真出图（可选）
...
cyx-shuohao-storyboard → storyboard.json （怎么拍：段、分镜、分镜图、H3 提示词）
```

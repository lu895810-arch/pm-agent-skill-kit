...
```
cyx-shuohao-outline    → outline.json    (what: structure)
cyx-shuohao-characters → cast.json       (who: character sheets)
cyx-shuohao-art        → art.json        (where: scene & prop sheets)
cyx-shuohao-script     → script.json     (the drama: scenes, beats, lines)
cyx-shuohao-storyboard → storyboard.json (how to shoot: segments, cuts, frames, H3 prompts)
```

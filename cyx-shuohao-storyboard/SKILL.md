---
name: cyx-shuohao-storyboard
version: 1.3.0
agent_created: true
display_name: "cyx 短剧分镜"
display_name_en: "cyx Short-Drama Storyboard"
description: |
...
**边界（不做的事）**：不写戏不改台词（`cyx-shuohao-script` 的活）、不出场景/角色/道具设定图（`cyx-shuohao-art` / `cyx-shuohao-characters` 的活）、不做视频生成与剪辑合成。口型/唇形同步暂不管——那是生成管线的事。
...
cyx-shuohao-characters → cast.json       （谁：角色设定图）
cyx-shuohao-art        → art.json        （哪里：场景/道具设定图）
cyx-shuohao-script     → script.json     （戏：场次、节拍、台词）
cyx-shuohao-storyboard → storyboard.json （怎么拍：镜头、首帧、批次）
```
...
**本技能不是一次写死的。** 用户拿本技能出分镜时提的每一条修改意见，都可能是适用于**所有后续分镜**的规则。本技能自带 `.gates.jsonl` 门失败记录，**先用 `stats` 看数据再决定改什么**——「哪道门最常响」说明那条规则模型最常无视，该改的是措辞而非加码；「哪道门从没响过」要么是死门、要么规则已被内化。判定、留痕、回退规则见 `cyx-skill-evolution`。

**触发**：用户对分镜或报告提意见（段的切分、切点节奏、提示词、批次单）；同一道门 **≥2 次失败** → 改措辞或判据；我校验报错与实际不符 → 当场核判据。

**落点映射**：

| 反馈类型 | 典型原话 | 改本文件哪一节 |
|---|---|---|
| 参数与节奏 | 「切得太碎 / 太慢」「段上限要改」 | 生成步骤 +「边界」（段上限、切节奏区间都是参数） |
| 段与切的结构 | 「这两切该并」「拍没认领完」 | 段/分镜构造步骤 + 校验器 |
| 提示词语言 | 「提示词要中文 / 英文」 | 生成步骤 +「边界」`promptLang` |
| 质量门 | 「这道门老响」 | `stats` 汇总 + 门实现 + `selftest.mjs` 用例 |
| 报告呈现 | 「报告这块看不清」 | 渲染步骤 + 导出按钮 |

**沉淀后必做**：① 改门判据必须**同步补 `selftest.mjs` 击穿用例**；② 在 `cyx-skill-evolution/references/feedback-log.md` 追加一行（本技能无自带账本，写通用账本）；③ 一句话告知用户「这条记进 cyx-shuohao-storyboard 了」。

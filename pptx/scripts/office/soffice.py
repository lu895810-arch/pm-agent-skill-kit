...
def _resolve_soffice() -> str:
    """Locate the soffice executable without depending on it being on PATH.

    On Windows LibreOffice installs to
    ``C:\\Program Files\\LibreOffice\\program\\soffice.exe``; probe the usual
    locations before falling back to the bare ``soffice`` name (which still
    works when it is on PATH, e.g. after the user PATH was updated).
    """
...
def run_soffice(args: Iterable[str], **kwargs) -> subprocess.CompletedProcess:
    ...
    with contextlib.ExitStack() as stack:
        ...
        return subprocess.run([_resolve_soffice()] + args, env=get_soffice_env(), **kwargs)

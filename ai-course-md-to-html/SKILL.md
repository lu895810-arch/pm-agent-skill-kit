---
name: ai-course-md-to-html
description: Convert long-form Markdown (plan, spec, system design, RFC, runbook, postmortem, brainstorm, notes) into a single self-contained HTML page with Mermaid diagrams, step timelines, callouts, sidebar TOC. WorkBuddy-orange light+dark theme. Multi-language. Portable across WorkBuddy / Codex / Antigravity / any AI agent.
trigger: /ai-course-md-to-html
agent_created: true
display_name: "Markdown 转课程 HTML"
display_name_en: "AI Course MD→HTML"
---

# /ai-course-md-to-html
...
```
/ai-course-md-to-html <file.md>             # output <file>.html next to source
/ai-course-md-to-html <file.md> --out X.html # custom output path
/ai-course-md-to-html                       # if no arg, ask user which file
```

## Skill files (resolved relative to this SKILL.md)

- `template.html`   — HTML skeleton with embedded CSS (WorkBuddy orange light+dark), Mermaid CDN, theme toggle, TOC sidebar, footer. Contains `{{PLACEHOLDER}}` strings and `<!-- COMMENT -->` slots.
...
- **WorkBuddy** — install at `~/.workbuddy/skills/ai-course-md-to-html/` (this directory, symlinked or copied). Invoke with `/ai-course-md-to-html <file>`.
- **Codex CLI** — copy `SKILL.md` content to `~/.codex/prompts/ai-course-md-to-html.md`, keep `template.html` and `components.md` at a stable absolute path, update the file references in SKILL.md if needed. Invoke with `/ai-course-md-to-html`.

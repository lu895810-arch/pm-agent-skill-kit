---
name: cyx-shuohao-script
version: 1.2.0
agent_created: true
display_name: "cyx 短剧剧本"
display_name_en: "cyx Short-Drama Script"
description: |
  给 AI 短剧写剧本：把 cyx-shuohao-outline 的分集梗概落成结构化的场次 + 节拍流（动作节拍与台词行交替），
...
**边界（不做的事）**：不分镜头、无镜号、不写画面生成提示词、不出图、不配乐——分镜层的活一件不碰。不改大纲结构（砍线合人是 `cyx-shuohao-outline` 的活）；不做角色和场景设定（`cyx-shuohao-characters` / `cyx-shuohao-art` 的活）。

---

### Step 0 — 定输入与范围

**outline.json 是剧本的直接上游**（分集梗概、爽点落点、单集时长都在里面），标准流程从它开始。没有的话先问用户是否跑 `cyx-shuohao-outline`；用户坚持直接写，就问清集数与单集分钟数，手建骨架，对账门会明说跳过。
...
cyx-shuohao-script     → script.json  （戏：场次、节拍、台词）
```
...
`{baseDir}/examples/渡口-script.json`：《渡口》**全 6 集完整剧本**（9 场 123 句台词，每集冷开场兑现钩子、都落在 120 秒 ±15% 内），对着 cyx-shuohao-outline 与 cyx-shuohao-art 的样例全部质量门通过。当质量基准，也是自测夹具。

## 自我进化机制（遵循 `cyx-skill-evolution` 通用条款）

**本技能不是一次写死的。** 用户拿本技能写剧本时提的每一条修改意见，都可能是适用于**所有后续剧本**的规则。判定、留痕、回退规则见 `cyx-skill-evolution`。

**触发**：用户对剧本或报告提意见（节拍、台词、时长、场次切分、门是否误拦）；**同一道门反复误判** → 判据或阈值要改；我校验报错与用户直觉不符 → 当场核判据。

**落点映射**：

| 反馈类型 | 典型原话 | 改本文件哪一节 |
|---|---|---|
| 参数与语速 | 「时长算得不对」「语速要调」 | Step 0 +「边界」（时长是估算、容差 ±15%） |
| 节拍与场次 | 「这一场该拆」「节拍漏了」 | 节拍流构造步骤 + 校验器 |
| 台词写法 | 「VO 用错了」「台词太长」 | 台词本构造 +「边界」VO 规则 |
| 时长门 | 「这道门不该拦 / 该拦没拦」 | 校验 + `scripts/novel-script.mjs` + `selftest.mjs` 用例 |
| 报告呈现 | 「报告这块看不清」 | 渲染步骤 + `.gates.jsonl`（`stats` 看哪道门最常响） |

**沉淀后必做**：① 改门判据必须**同步补 `selftest.mjs` 击穿用例**；② 在 `cyx-skill-evolution/references/feedback-log.md` 追加一行（本技能无自带账本，写通用账本）；③ 一句话告知用户「这条记进 cyx-shuohao-script 了」。

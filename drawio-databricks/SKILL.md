...
upstream: "sparklabx/drawio-ai-kit (drawio-databricks)"
---

> **Buddy 适配（2026-09-24）**：已装入 WorkBuddy。引擎位于 `~/.workbuddy/drawio-ai-kit`，`drawio-ai` CLI 经 `~/.workbuddy/bin` 的 shim 已在 PATH（**无需** `npm i -g`）。下方 preflight 的 `command -v drawio-ai` 已通过。PNG 渲染需本机 draw.io 桌面 CLI，可选。

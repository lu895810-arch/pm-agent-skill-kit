---
name: cyx-shuohao-outline
version: 1.2.0
agent_created: true
display_name: "cyx 短剧大纲"
display_name_en: "cyx Short-Drama Outline"
description: |
...
**边界（不做的事）**：不写剧本台词、不做分镜、不出图像/TTS 提示词。梗概是叙述体，出现引号对白就是越界——`validate` 会拦。想从小说拆角色设定（画像/形象提示词/设定图），那是 `cyx-shuohao-characters` 的活。
...
**人物表从原文拆**——大纲是角色设定的上游，`characters` 块定下的分档、人物线与来源，下游 `cyx-shuohao-characters` 直接拿去当角色清单，不用再判断一遍谁重要。

例外是用户手上已经有 `cast.json`（此前单独跑过 `cyx-shuohao-characters`）：那就拿来当人物原料，角色、别名、关系都是现成的，不用重拆原文。分档按 `importance` 反向映射：protagonist/major → `lead`，supporting → `support`，minor → `functional`。
...
`{baseDir}/examples/渡口-outline.json`：把短故事《渡口》（cyx-shuohao-characters 的自带样例）改编成 6 集 × 2 分钟的微型大纲，四角色三场景四爽点，全部质量门通过。当质量基准，也是自测夹具。

## 自我进化机制（遵循 `cyx-skill-evolution` 通用条款）

**本技能不是一次写死的。** 用户拿本技能改编时提的每一条修改意见，都可能是适用于**所有后续改编**的规则——尤其「质量门该不该拦、拦在哪」这类判断，值得沉淀。判定、留痕、回退规则见 `cyx-skill-evolution`。

**触发**：用户对大纲或评审报告提意见（爽点节奏、人物分档、集数切分、门是否误拦）；**同一道门反复误判** → 阈值应当参数化或判据要改；我校验时报错与用户直觉不符 → 当场核判据。

**落点映射**：

| 反馈类型 | 典型原话 | 改本文件哪一节 |
|---|---|---|
| 参数与口径 | 「集数/时长该是…」「分档上限不对」 | Step 0（收参数）+「边界」（阈值是参数不是圣旨） |
| 骨架与梗概写法 | 「这几集该合并」「梗概不够抓人」 | Step 3–5 |
| 质量门判据 | 「这道门不该拦 / 该拦没拦」 | Step 6 + `scripts/novel-outline.mjs` 的门实现 + `selftest.mjs` 用例 |
| 联动更新 | 「改了上游下游没跟上」 | 「联动更新」（靠校验器不靠记忆） |
| 报告呈现 | 「报告上这块看不清」 | Step 7 + 渲染器 |
| 多语言 | 「界面要英文」 | 「边界」`--lang` 项 |

**沉淀后必做**：① 改门判据必须**同步补一条 `selftest.mjs` 击穿用例**（否则下次改脚本会静默失效）；② 在 `cyx-skill-evolution/references/feedback-log.md` 追加一行（本技能无自带账本，写通用账本）；③ 一句话告知用户「这条记进 cyx-shuohao-outline 了」。

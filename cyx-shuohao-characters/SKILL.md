---
name: cyx-shuohao-characters
version: 1.11.0
agent_created: true
display_name: "cyx 短剧角色"
display_name_en: "cyx Short-Drama Characters"
description: |
  ...
  零依赖、零 API key，用当前会话额度；出图优先走 WorkBuddy 原生 ImageGen（codex $imagegen 回退，可选）。
...
metadata:
  ...
  requires:
    ...
    optional:
      - codex         # 仅作回退：默认用 WorkBuddy 原生 ImageGen 出三视图；无 ImageGen 且有 codex 时才回退；两者皆无则跳过只交提示词
  runtimes:
    - workbuddy
---
...
**运行环境**：WorkBuddy 和 codex 都能跑。差别只在第 8 步出图——见 `references/sheet.md`。
...
**有 `outline.json`（cyx-shuohao-outline 的产出）就一起要过来，走 seed**——大纲是角色设定的上游，它的 `characters` 块已经定死了角色清单：
...
**当前环境支持子代理就并发**（WorkBuddy 的 Task、codex 的 subagent）：每块一个子代理，**所有调用放在同一条消息里**才是真并发。不支持就一块一块串行读，结果一样，只是慢。
...
**本技能不是一次写死的。** 用户拿本技能出角色设定集时提的每一条修改意见，都可能是适用于**所有后续改编**的规则。判定、留痕、回退规则见 `cyx-skill-evolution`。

**触发**：用户对角色表、画像、提示词或报告提意见（语言、分档、出图要求、归并结果）；**同一类校验报错反复出现** → 说明提示词或 schema 该改；我校验时报错与实际情况不符 → 当场核判据。

**落点映射**：

| 反馈类型 | 典型原话 | 改本文件哪一节 |
|---|---|---|
| 参数与语言 | 「报告要英文」「默认取前 30 太多」 | Step 0 +「边界」 |
| 分块与归并 | 「这两个是同一个人」「该合的没合」 | Step 4 + `scripts/novel-characters.mjs` 的归并逻辑 |
| 角色卡写法 | 「画像太笼统」「引文不对」 | Step 6 + `references/profile-pass.md` / `references/schema.md` |
| 校验项 | 「这类错该拦没拦」 | Step 7 + 校验器 + `selftest.mjs` 用例 |
| 出图规格 | 「三视图脸不一致」「人物被压扁」 | Step 8 + `references/sheet.md` |
| 报告呈现 | 「报告版式不对」 | Step 9 + `references/report-style.md` |

**沉淀后必做**：① 改校验与 schema 必须**同步补 `selftest.mjs` 用例**；② 在 `cyx-skill-evolution/references/feedback-log.md` 追加一行（本技能无自带账本，写通用账本）；③ 一句话告知用户「这条记进 cyx-shuohao-characters 了」。

...
upstream: "yizhiyanhua-ai/fireworks-tech-graph"
---

> **Buddy 适配（2026-09-24）**：已装入 WorkBuddy。脚本用托管 `python3`（已在 PATH）运行；`SKILL_ROOT` 即本目录，`python3 scripts/fireworks.py version` 可自动探测。SVG / 离线 HTML 开箱即用；PNG 导出需本机 `rsvg-convert`，GIF 动效需 Puppeteer（Node 已满足 ≥22.12），二者均按需安装。

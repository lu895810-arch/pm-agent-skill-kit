---
name: cyx-shuohao-art
version: 1.2.0
agent_created: true
display_name: "cyx 短剧美术"
display_name_en: "cyx Short-Drama Art"
description: |
  ...
  有 cyx-shuohao-outline 的 outline.json 就用 seed 预填场景清单与出现集；出图走 codex 内置 $imagegen（可选）。
...
**边界（不做的事）**：不做分镜、不写剧本、不管角色（`cyx-shuohao-characters` 的活）、不排大纲（`cyx-shuohao-outline` 的活）。**道具只收叙事道具**（有特写、跨集、承载剧情的，通常 3–8 件）——场景陈设归场景锚点，一次性手部道具镜头级提示词解决，都不单独建资产。
...
1. **outline.json**（cyx-shuohao-outline 的产出）——最优，场景清单、出现集、承载爽点、复用方案都是现成的
2. 小说原文——自己归纳场景清单（主舞台优先，参考 cyx-shuohao-outline 的主场景上限思路：别贪多）
3. 用户手写的场景清单
...
有 cast.json（cyx-shuohao-characters 的产出）也带上——校验「提示词不含角色名」要用。
...
```
cyx-shuohao-outline    → outline.json （什么：结构与分集）
cyx-shuohao-characters → cast.json    （谁：角色资产）
cyx-shuohao-art        → art.json     （哪里 + 手里拿的：美术资产）
```
...
- 场景数量不设硬上限——上限在 cyx-shuohao-outline 的主场景门那里管；这里管的是每个资产的质量
...
`{baseDir}/examples/渡口-art.json`：《渡口》三场景 + 两件叙事道具（旧皮箱、县衙旧砚）的完整设定，全部质量门通过（含对着 cyx-shuohao-characters 样例 cast 的角色名检查）。当质量基准，也是自测夹具。

## 自我进化机制（遵循 `cyx-skill-evolution` 通用条款）

**本技能不是一次写死的。** 用户拿本技能出美术设定时提的每一条修改意见，都可能是适用于**所有后续设定集**的规则。判定、留痕、回退规则见 `cyx-skill-evolution`。

**触发**：用户对场景、道具、提示词或报告提意见；**同一道质量门反复误判** → 判据要改；我校验报错与实际不符 → 当场核判据。

**落点映射**：

| 反馈类型 | 典型原话 | 改本文件哪一节 |
|---|---|---|
| 画风与语言 | 「画风要和角色那边一致」「提示词要英文」 | 生成步骤 +「边界」 |
| 场景设定 | 「这个场景该有光照变体」「空景提示词不对」 | 场景构造步骤 + `references/` 下对应说明 |
| 道具范围 | 「这件不算叙事道具」 | 道具构造步骤 +「边界」（只收叙事道具，3–8 件） |
| 质量门 | 「这道门不该拦 / 该拦没拦」 | 校验 + `scripts/novel-art.mjs` + `selftest.mjs` 用例 |
| 报告呈现 | 「报告版式不对」 | 渲染步骤 + 导出按钮 |

**沉淀后必做**：① 改门判据必须**同步补 `selftest.mjs` 击穿用例**；② 在 `cyx-skill-evolution/references/feedback-log.md` 追加一行（本技能无自带账本，写通用账本）；③ 一句话告知用户「这条记进 cyx-shuohao-art 了」。
